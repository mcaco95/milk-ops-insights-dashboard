...
DROP TABLE IF EXISTS samsara_addresses;
...
CREATE TABLE routes_data (
    ...
    samsara_route_name TEXT,       -- Full Samsara route name for reference and parsing
    report_date DATE NOT NULL,
    
    -- Route Details
    driver_name VARCHAR(255),
    truck_id VARCHAR(100),
    status VARCHAR(50), -- e.g., 'scheduled', 'en_route', 'filling_tank', 'completed'
    
    -- ENHANCED: Separate timestamp fields for different stages
    depot_departure_time TIMESTAMPTZ,  -- When truck left depot
    dairy_arrival_time TIMESTAMPTZ,    -- When truck arrived at dairy
    dairy_departure_time TIMESTAMPTZ,  -- When truck left dairy
    estimated_eta TIMESTAMPTZ,         -- Calculated ETA based on status
    
    -- Legacy fields (kept for backwards compatibility)
    estimated_arrival TIMESTAMPTZ,     -- Legacy field - use specific fields above instead
    start_date TIMESTAMPTZ,           -- Legacy field - use depot_departure_time instead
    
    -- Route Information
    route VARCHAR(255),
    dairy_name VARCHAR(255),
    tank VARCHAR(255),                 -- Legacy tank field
    samsara_tank VARCHAR(50),          -- Tank number extracted from Samsara route name
    processor VARCHAR(255),
    lt_number VARCHAR(255),
    fairlife_number VARCHAR(255),
    tracking_link TEXT,
    last_updated TIMESTAMPTZ DEFAULT (now() AT TIME ZONE 'UTC'::text)
);

CREATE INDEX idx_routes_data_dairy_id_date ON routes_data(dairy_id, report_date);
CREATE INDEX idx_routes_data_status ON routes_data(status);
CREATE INDEX idx_routes_data_lt_number ON routes_data(lt_number);
...
-- =================================================================
--  Samsara Addresses Table
--  Stores addresses from Samsara API for location-based functionality.
-- =================================================================
...
CREATE INDEX idx_samsara_addresses_name ON samsara_addresses(name);
CREATE INDEX idx_samsara_addresses_location ON samsara_addresses(latitude, longitude);
COMMENT ON TABLE samsara_addresses IS 'Stores addresses from Samsara API to avoid repeated API calls.';

-- Insert Samsara addresses
