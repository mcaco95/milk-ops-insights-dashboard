...
def process_load_summaries(summaries, dairy_id_map=None):
    """
    ...
    for (dairy, handler), volume in grouped_data.items():
        # Map dairy name to database ID if mapping provided
        if dairy_id_map and dairy in dairy_id_map:
            mapped_dairy_id = dairy_id_map[dairy]
            volume_by_dairy_handler[mapped_dairy_id][handler] += volume
        else:
            # If no mapping or dairy not found in mapping, use original name
            if dairy_id_map:
                print(f"Warning: Could not find a matching dairy_id for producer '{dairy}'. Skipping.")
                continue
